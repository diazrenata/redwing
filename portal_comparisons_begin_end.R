...
sites <- read.csv(here::here("portal_nulls", "white.csv")) %>%
  select(siteID) %>%
  distinct()
...
setup <- drake::drake_plan(
  mammal_sds = target(read.csv(here::here("portal_nulls", "white_splist.csv"))),
...
real <- drake::drake_plan(
  real_counts = target(read.csv(here::here("portal_nulls", "white.csv"))))
