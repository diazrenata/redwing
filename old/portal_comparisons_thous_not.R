...
sites <- read.csv(here::here("portal_nulls", "thous_not.csv")) %>%
  select(siteID) %>%
  distinct()
...
setup <- drake::drake_plan(
  mammal_sds = target(read.csv(here::here("portal_nulls", "thous_not_splist.csv"))),
...
real <- drake::drake_plan(
  real_counts = target(read.csv(here::here("portal_nulls", "thous_not.csv"))))
...
db <- DBI::dbConnect(RSQLite::SQLite(), here::here("drake-cache-pn-t-not.sqlite"), synchronous = NULL)
