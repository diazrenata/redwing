...
bp <- drake::drake_plan(
  bird_pairs = target(make_bird_pairs(sites))
)
...
pulled_datasets <- drake::drake_plan(
  d = target(pull_bird_dat(matss_dataset = ds, timedesc = "end"),
...
real_describe <- drake::drake_plan(
  siteDescs = target(describe_bird_counts(bird_counts = bc, all_bird_spp = all_sp),
...
sim_describe <- drake::drake_plan(
  siteDescs = target(describe_bird_counts(bird_counts = bc, shuffled_sp = all_sp, all_bird_spp = all_sp),
...
db <- DBI::dbConnect(RSQLite::SQLite(), here::here("drake-cache-bird-pairs-not.sqlite"), synchronous = NULL)
