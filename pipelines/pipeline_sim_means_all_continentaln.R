...
null_datasets <- drake_plan(
  nulldat = target(rwar::shuffle_continental(dataset,  null_mod_seed = null_seed),
                   transform = cross(
                     dataset = !!rlang::syms(datasets$target),
...
db <- DBI::dbConnect(RSQLite::SQLite(), here::here("aspirational_structure", "drake_caches", "all_contn.sqlite"))
...
#
 ...
 save(all_sims, all_winners,  all_qis, all_diagnostics, file = "portable_results_all_contn.Rds")
rm(all_sims)
rm(all_winners)
rm(all_qis)
rm(all_diagnostics)
