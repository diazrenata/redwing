...
begin <- c(1, 1, 1, 3, 3, 3, 5, 5, 5)

end_even <- c(1, 1, 3, 3, 5, 5)

end_small <- c(1, 1, 1, 1, 1, 1)

end_big <- c(3, 3, 5, 5, 5, 5)


biomass_outcomes <- data.frame(
  timeperiod = c(1:2),
  abundance = c(9, 6),
  even = c(sum(begin), sum(end_even)),
  small = c(sum(begin), sum(end_small)),
  big = c(sum(begin), sum(end_big))
) %>%
  tidyr::pivot_longer(-c(timeperiod, abundance), names_to = "scenario", values_to = "biomass")

ggplot(biomass_outcomes, aes((timeperiod), biomass)) + geom_line() + facet_wrap(vars(scenario)) + geom_line(aes(y = abundance), color = "blue")
...
```{r, fig.dim = c(10, 3), results = T}

dat <- MATSS::get_bbs_route_region_data(route = 224, region = 3)

dat_isd <- just_isd(dat, isd_seed = 1989)
...
ggplot(filter(isd_inds), aes((mass), fill = timeperiod)) + geom_histogram() + facet_wrap(vars(timeperiod), ncol = 1) + scale_x_log10() + ylab("Number of individuals") + xlab("Mass (g); note log scale") + theme(legend.position = "none")


dat_to_plot <- isd_inds %>%
  group_by(year) %>%
  summarize(
...
`draw_communities(ts_comp, begin_years = 1988:1992, end_years = 2014:2018, draw_seed = 1989, sampling_gmms = sampling_gmms)`
...
- What we are actually interested in for understanding decoupling is the deviation between "currency" - blue lines - and "abundance" - red lines - within plots for energy and biomass.
- The "abundance" shows the change in biomass/energy use that occurs if the size structure remains the same from beginning to end, i.e. only driven by the change in individual abundance between the two time periods.
- The "currency" shows the change incorporating observed change in the size structure.
