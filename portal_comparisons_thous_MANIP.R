...
sites <- read.csv(here::here("portal_nulls", "thous.csv")) %>%
  select(siteID) %>%
  distinct()
...
setup <- drake::drake_plan(
  mammal_sds = target(read.csv(here::here("portal_nulls", "thous_splist_MANIP.csv"))),
...
real <- drake::drake_plan(
  real_counts = target(read.csv(here::here("portal_nulls", "thous.csv"))))
