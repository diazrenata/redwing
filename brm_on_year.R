...
granby = granby
granby_sgmms <- construct_sampling_gmm(granby)

multi_svs <- draw_communities_wrapper(granby, ndraws = 10, sampling_gmms = granby_sgmms)

fit_dat <- multi_svs %>% filter(source != "raw")

brm_tp <- brm(total_energy ~ timeperiod * source + (1 | year), data = fit_dat)

brm_y <- brm(total_energy ~ year * source, data = fit_dat)
...
td_ests <- td %>%
  ...
  dplyr::group_by_all() %>%
  dplyr::mutate(
    estimated_actual_begin = sum(b_Intercept, (1990 * b_year)), # estimated beginning value
    estimated_actual_end = sum(b_Intercept, (2016 * b_year)), # estimated end value
    estimated_sim_begin = sum(b_Intercept, b_sourcesim, ((b_year + year_sourcesim) * 1990)), # estimated beginning value from sims. we expect this to be equal to the estimated beginning value, any change is just sampling error.
    estimated_sim_end = sum(b_Intercept, b_sourcesim, ((b_year + year_sourcesim) * 2016)),
...
tp_ests <- extract_ests(brm_tp)


ggplot(td_ests, aes(estimated_sim_begin)) + geom_density() + geom_density(data = tp_ests, color = "blue")

ggplot(td_ests, aes(estimated_actual_begin)) + geom_density() + geom_density(data = tp_ests, color = "blue")


ggplot(td_ests, aes(estimated_actual_change_ratio)) + geom_density() + geom_density(data = tp_ests, color = "blue")


  ggplot(td_ests, aes(estimated_sim_change_ratio)) + geom_density() + geom_density(data = tp_ests, color = "blue")
...
ggplot(td_ests, aes(estimated_change_ratio_deviation)) + geom_density()+ geom_density(data = tp_ests, color = "blue")
