...
Testing the relationship between change in community-level abundance and community function (biomass and energy use), as mediated by change in the individual size distribution. 

This is the first macroecological-scale study of the interrelated dynamics of ISDs, abundance, and function for terrestrial animal communities. 
...
We simulated ISDs for the time periods 1988-1992 and 2014-2018 for 528 routes in the Breeding Bird Survey. We evaluated 1) the extent of change in the ISDs from the beginning to the end time periods; 2) the change in total biomass and total energy use over this time span; 3) the extent to which the observed changes in biomass and energy use reflect changes in numerical *abundance* vs. change in the ISD. 

## Results

Nearly all ISDs have changed detectably over time, but in most instances this does not result in a decoupling between biomass/energy use and numerical abundance. However, in a substantial minority of communities - 30% of routes overall for biomass, and 14% overall for energy use - directional shifts in the ISD result in a decoupling. In 80% of instances of decoupling, the deviation is towards an increase in body size/per capita metabolic rate, resulting in a less negative change in function than the change driven by numerical abundance. 

## Main conclusions

Terrestrial animal ISDs are dynamic; these dynamics have consequences for the scaling of abundance and function; and this study provides tools and motivation for examining these dynamics!
...
```{r, results=T, include=T, fig.dim =c(5, 2.5)}

real_plot <- ggplot(actual_isd, aes(mass, density, color = timeperiod)) + geom_line() + ggtitle("Actual") + theme(legend.position="none")

sim_plot <- ggplot(bootstrap_isd, aes(mass, density, color = timeperiod)) + geom_line() + ggtitle("Shuffled (one shuffle)")+ theme(legend.position="none")

isd_panels <- multi_panel_figure(columns = 2, rows =1) %>%
  fill_panel(real_plot) %>%
  fill_panel(sim_plot) 
...
biom <- ggplot(ssims, aes(year, total_biomass, color = source)) + geom_point() + geom_smooth(method = "lm", se = F) + ggtitle("Biomass") + theme(legend.position = "bottom")

ener <- ggplot(ssims, aes(year, total_energy, color = source)) + geom_point() + geom_smooth(method = "lm", se = F) + ggtitle("Energy use") + theme(legend.position = "bottom")



change_panels <- multi_panel_figure(columns = 2, rows =1) %>%
  fill_panel(biom) %>%
  fill_panel(ener) 
...
slopeplot<-ggplot(these_qis, aes(timeperiodend, currency)) + geom_pointinterval(aes(xmin = timeperiodend.lower, xmax = timeperiodend.upper)) + ggtitle("Slope term") + geom_vline(xintercept = 0)


offsetplot <-ggplot(these_qis, aes(`timeperiodend:sourcecurrency`, currency)) + geom_pointinterval(aes(xmin = `timeperiodend:sourcecurrency.lower`, xmax = `timeperiodend:sourcecurrency.upper`)) + ggtitle("Offset due to currency term") + geom_vline(xintercept = 0)


par_panels <- multi_panel_figure(columns = 2, rows =1) %>%
  fill_panel(slopeplot) %>%
  fill_panel(offsetplot) 
...
I subsampled the 528 routes to get a maximum of 10 routes per bird conservation region, so that the highly-sampled BCRs don't  dominate aggregate analyses [@thibault2011]. That yields 238 routes total. Results are the same; I haven't included them in this document.
...
sig_changes <- raw_changes %>%
  ...
  mutate(model_key = ifelse(
    grepl("full", modtype), "Decoupling",
    ifelse(grepl("source", modtype), "Change due to abundance", "No slopes")
  ))




e_bigplot <- ggplot(filter(sig_changes, currency == "energy"), aes(energy_ratio_abundance, energy_ratio_currency, color = model_key)) + geom_point(alpha = .5) + geom_abline(slope= 1, intercept = 0) + geom_vline(xintercept = 1) + geom_hline(yintercept = 1) + theme(legend.position = "right", legend.direction = "vertical") + ggtitle("Energy") + xlab("Change due to abundance\n(end value) / (begin_value)") + ylab("Change due to abundance +\nchange in ISD\n(end_value) / (begin_value)")



b_biglot <- ggplot(filter(sig_changes, currency == "biomass"), aes(biomass_ratio_abundance, biomass_ratio_currency, color = model_key)) + geom_point(alpha = .5) + geom_abline(slope= 1, intercept = 0) + geom_vline(xintercept = 1) + geom_hline(yintercept = 1) + theme(legend.position = "right", legend.direction = "vertical") + ggtitle("Biomass")+ xlab("Change due to abundance\n(end value) / (begin_value)") + ylab("Change due to abundance +\nchange in ISD\n(end_value) / (begin_value)")
...
winners_comps <- winners_comps %>%
  ...
  mutate(logratio_mean_size_squared = logratio_mean_size ^ 2,
         ...
         abs_logratio_mean_size = abs(logratio_mean_size))


winners_comps_distinct <- select(winners_comps, matssname, overlap, logratio_mean_size, logratio_mean_size_squared, logratio_mean_e, logratio_mean_e_squared,abs_logratio_mean_size) %>% distinct()
...
meansize_lograt_lm <- lm(abs_logratio_mean_size ~ overlap, data = winners_comps_distinct)
meansize_lograt_summ <- summary(meansize_lograt_lm)

winners_comps_distinct <- winners_comps_distinct %>%
  mutate(meansize_pred = predict(meansize_lograt_lm))

ggplot(winners_comps_distinct, aes(overlap, abs_logratio_mean_size)) + geom_point() + geom_line(aes(y = meansize_pred)) + 
  xlab("ISD overlap\nRanges 0-1, 1 is less change") + 
  ylab("Absolute log ratio of mean size\nabs(log(mean_size_end/ mean_size_end))") + geom_text(aes(.85, .4, label = paste0("R2 = ", signif(meansize_lograt_summ$r.squared, 3))), data = winners_comps_distinct[1,])+ geom_text(aes(.85, .45, label = "p < 0.001"), data = winners_comps_distinct[1,])

```

##### Figure 2. Change in the ISD vs. change in mean size.

*Axes:* The x axis is the ISD overlap for a route (ranges 0-1, with 1 being identical ISDs), and the y axis is the absolute log ratio of mean body size for the end vs the beginning. The points are individual routes; the line is the prediction from a quadratic model fitting `absolute_log_ratio ~ isd_overlap`. 

*Interpretation*: The relationship between overall change in the ISD (overlap) and change *in mean body size* is a very messy one; R2 = .06. It's not possible to have a *lot* of change in mean body size if there is very high ISD overlap, hence the slight negative relationship. However, for any given amount of change in the ISD there is a lot of variability in the effect on mean size. 
...
load(here::here("results", "results_objects", "portable_results_core.Rds"))
...
1. Diagnosing process
    ...
    1. Disentangling process-driven shifts from drift. 
        1. Null models at this scale are severely constrained in what they can tell us.
        ...
        1. That said, I don't generally find deviations from null models in these results (although I am not sure because the computational requirements are too great to run this very many times).
...
1. The current method cannot account for **intraspecific** size variation over time or space, which may be important. 

## Wider significance


1. The interrelated dynamics of the size structure, total abundance, and community function have a rich history in aquatic and tree systems. This study is a first step towards translating this approach to terrestrial animal systems.
1. We show that terrestrial animal ISDs change over time, and that these changes can have significant cascading effects on the scaling between abundance and size-based measures of function.
1. Incorporating the ISD is important to fully characterize biodiversity change, especially in the Anthropocene.
1. Our study provides the motivation and a quantitative approach for studying these dynamics, which can be applied either in greater depth (to understand the processes driving change in specific communities) or in novel taxonomic or geographic contexts (e.g. mammals, birds beyond BBS). 
1. We hope that this can launch a new chapter of research on the size spectrum and its relation to community function in terrestrial systems. 
