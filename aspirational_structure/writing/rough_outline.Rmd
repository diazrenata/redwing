...
actual_winners_boxplot <- actual_winners %>% group_by(currency, model) %>% summarize(n = dplyr::n(),
...
For about 60% of models *with a change beginning to end*, abundance is decreasing. Note that the number that are increasing vs. decreasing can sum to less than 1 if there are models with an interaction in which the abundance-slope is over 0, but the currency offset is nonzero. 
...
```{r, results = T, include = T, fig.dim = c(5,4)}
...
raw_changes <- all_sims %>%
  ...
  mutate(energy_ratio = (total_energy_end - total_energy_begin) / total_energy_begin,
         biomass_ratio = (total_biomass_end - total_biomass_begin) / total_biomass_begin) %>%
...
ggplot(sig_changes, aes(energy_ratio_abundance, energy_ratio_currency)) + geom_point(alpha = .51) + geom_point(data =filter(sig_changes, abund_slope_nonzero), aes(color = currency_offset_nonzero), alpha = 1) + geom_abline(slope= 1, intercept = 0) + geom_vline(xintercept = 0) + geom_hline(yintercept = 0) + theme(legend.position = "bottom") + ggtitle("Energy")

```

```{r, results = T, include = T, fig.dim = c(5,4)}

ggplot(sig_changes, aes(biomass_ratio_abundance, biomass_ratio_currency)) + geom_point(alpha = .51) + geom_point(data =filter(sig_changes, !is.na(timeperiodend)), aes(color = currency_offset_nonzero), alpha = 1) + geom_abline(slope= 1, intercept = 0) + geom_vline(xintercept = 0) + geom_hline(yintercept = 0) + theme(legend.position = "bottom") + ggtitle("Biomass")
```

## Relating change in the ISD to decoupling

There's a lot of change in the ISD that doesn't translate into decoupling.

```{r}

winners_comps <- left_join(actual_winners, all_comps)

ggplot(winners_comps, aes(overlap, fill = currency)) + geom_density() + facet_wrap(vars(currency, model), scales = "free", ncol = 2, dir = "v") + theme(legend.position = "none")

ggplot(winners_comps, aes(log(end_mean_size / begin_mean_size), overlap)) + geom_point()

ggplot(winners_comps, aes(log(end_mean_e / begin_mean_e), overlap)) + geom_point()
```

## Core-transient

Everything stays the same **except for** the 80%.
