...
ggplot(one_isd, aes(log(mass), fill = timeperiod)) + geom_density(alpha = .5) + ggtitle("Actual") 

ggplot(shuffled_isd, aes(log(mass), fill = timeperiod)) + geom_density(alpha = .5) + ggtitle("Shuffled (one shuffle)")
...
For this site, the KS test p value is `r round(isd_comparison$actual_p[1], 4)`. The observed test statistic has a percentile of `r round(isd_comparison$d_percentile[1],2)` and  SES of `r round(isd_comparison$d_ses[1], 2)` compared to 500 shuffles. The observed ISDs have an overlap of `r round(isd_comparison$overlap, 2)`.
...
1. Simulate change in total energy and total biomass under "no change in ISD" and "observed change in ISD" scenarios
      1. We construct sampling GMM for each time period.
          1. The "sampling GMM" is a probability density function with the probability of observing an individual of a given mass across the spectrum of possible masses. Sampling `$mass` with probability `$density` is a random number generator for sizes.
          1. We construct this by drawing individuals (based on their species' abundances and mean and SD body sizes), fitting a GMM, and then rescaling the density function of the GMM so it sums to 1.
          1.For this, because there is some sampling error, I draw 5 copies of the ISD and fit the GMM to the combined 5 draws. This doesn't affect anything in practice and is mostly inspired by a one-off comment from Allen Hurlbert about "sampling error" so I might drop it. We construct sampling ISDs for the "begin" and "end" time periods.
...
raw_sims <- make_actual_sims(dat = dat, initial_isd_seed_gmm = 1989, n_isd_draws = 1, ndraws = 5, initial_draw_seed = 1989) %>%
  mutate(sim_iteration = paste0(sim_iteration, source))

ssims <- ssims_wrapper(dat, "actual", 1989, 1, 5, 1989)

```

```{r, results=T, include=T, fig.dim = c(3.5, 3.5)}

ggplot(raw_sims, aes(year, total_biomass, color = source, group = as.character(sim_iteration))) + geom_point() + geom_smooth(method = "lm", se = F) + ggtitle("Five sims") + theme(legend.position = "bottom")

ggplot(ssims, aes(year, total_biomass, color = source)) + geom_point() + geom_smooth(method = "lm", se = F) + ggtitle("Five sims summarized to mean") + theme(legend.position = "bottom")

```

Here, the "abundance" lines (red) reflect change in total biomass expected due only to changes in abundance. The "currency" lines reflect change in total biomass due to change in abundance *and* observed change in the size structure. Deviations in slope between the red and blue lines reflect change driven by the ISD. Note that the lines here are crude linear model smooths and not really suitable for inference, just visualization. 
