...
#datasets <- datasets[ unique(c(1:325, which(datasets$target %in% c("bbs_rtrg_224_3", "bbs_rtrg_318_3", "bbs_rtrg_19_7", "bbs_rtrg_116_18", "bbs_rtrg_3_80")))), ]
...
methods <- drake_plan(
  ssims = target(rwar::local_null_model(dataset, null_mod_seed = 1989, n_isd_draws = 5, ndraws = 5),
                 transform = cross(
...
db <- DBI::dbConnect(RSQLite::SQLite(), here::here("aspirational_structure", "drake_caches", "local_null1.sqlite"))
...
#
 ...
 save(all_sims, all_winners,  all_qis, all_diagnostics, file = "portable_results_local_null1.Rds")
