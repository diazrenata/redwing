...
methods <- drake_plan(
  ssims = target(rwar::ssims_wrapper(dataset, simtype, n_isd_draws = 5, ndraws = 5),
                 transform = cross(
                   dataset = !!rlang::syms(nontransient_datasets$target),
...
all = bind_rows(datasets,nontransient_datasets,  methods)


## Set up the cache and config
db <- DBI::dbConnect(RSQLite::SQLite(), here::here("aspirational_structure", "drake_caches", "nontransient.sqlite"))
...
#
 ...
 save(all_sims, all_winners,  all_qis, all_diagnostics, file = "portable_results_nontransient.Rds")
