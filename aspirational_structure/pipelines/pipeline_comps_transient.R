...
methods <- drake_plan(
  coredat = target(rwar::just_transient(dataset, transient_only = T),
                   transform = map(
                     dataset = !!rlang::syms(datasets$target)
                   )),
...
db <- DBI::dbConnect(RSQLite::SQLite(), here::here("aspirational_structure", "drake_caches", "comps_transient.sqlite"))
...
#
 ...
 save(all_comps, file = "portable_comps_transient.Rds")
